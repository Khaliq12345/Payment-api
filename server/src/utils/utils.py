...
def get_user(email: str) -> dict:
    """Get a user based on emails"""
    try:
        with open("./src/clients.json", "rb") as f:
            json_string = f.read()
            json_dict = json.loads(json_string)
            return json_dict[email]
    except Exception as _:
        return {}


def add_user(email: str, user_id: str, number: str):
    filepath = "./src/clients.json"
    ...
    # Update the group
    data[email] = {"user_id": user_id, "number": number}
