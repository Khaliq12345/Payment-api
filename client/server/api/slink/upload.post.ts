...
export default defineEventHandler(async (event) => {
  ...
  const response = await fetch(
    `${config.slinkUrl}/api/external/upload`,
    {
      method: "POST",
      headers: {
        ...slinkForm.getHeaders(),
  ...
  if (!response.ok) {
    ...
    if (duplicate) {
      ...
      return {
        url: `${config.slinkUrl}/image/${imageId}.${extension}`,
        id: imageId,
      };
    }
  ...
  return {
    url: `${config.slinkUrl}/image/${result.id}.${extension}`,
    id: result.id,
  };
});
